...
class ScheduleController:
    ...
    def aggregate_day_schedule(self, weekday: int, group: str):
        day_schedule: dict[str, str] = SCHEDULE[group][str(weekday + 1)]
        aggregated_day_schedule: dict[str, list[int]] = dict()

        schedule_pairs = sorted(
            [(value, int(key) - 1) for key, value in day_schedule.items()],
            key=lambda d: d[1],
        )
        ...
        for state, timing in aggregated_day_schedule.items():
            new_timing = [
                timing[i] if helper[state][i] != 1 else None for i in range(len(timing))
            ]
            aggregated_day_schedule[state] = list(filter(lambda i: i, new_timing))
    ...
    def _next_zone_datetime_by_type(self, group: str, zone_type: str) -> datetime:
        ...
        return (
            ...
            .replace(hour=tomorrows_zone_timing[0], minute=0, second=0, microsecond=0)
            .astimezone(pytz.utc)
        )

    def get_next_zone_type_and_time(
        self, group: str
    ) -> Tuple[ElectricityStateLog.STATE, datetime]:
    ...
    def timedelta_to_next_recovery(self, group: str) -> timedelta:
        ...
        return (
            next_recovery - datetime.now(pytz.utc)
    ...
    def timedelta_to_next_blackout(self, group: str) -> timedelta:
        ...
        return (
            next_blackout - datetime.now(pytz.utc)
